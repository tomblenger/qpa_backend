...
class RolePermissionController {
  async updateRolePermission(req, res) {
    try {
      ...
      const updatedRole = await RolePermissionService.updateRolePermissions(roleId, permissionIds);
      res.status(200).json({ message: 'Project updated successfully', Role: updatedRole });
    } catch (error) {
